...
@section('ticketit_content')
    {!! CollectiveForm::open([
        ...
        <div class="form-group row">
            {!! CollectiveForm::label('image', trans('ticketit::lang.image') . trans('ticketit::lang.colon'), ['class' => 'col-lg-2 col-form-label']) !!}
            <div class="col-lg-10">
                    <input type='file' id="image" name="image[]" accept=".png, .jpg, .jpeg" multiple />
                {{-- {!! CollectiveForm::file('image[]', null, ['class' => 'form-control', 'required' => 'required', 'accept' => '.png , .jpg , .jpeg', 'multiple' => 'multiple']) !!}
                {!! CollectiveForm::file('image[]', null, ['class' => 'form-control', 'required' => 'required', 'accept' => '.png , .jpg , .jpeg', 'multiple' => 'multiple']) !!}
                {!! CollectiveForm::file('image[]', null, ['class' => 'form-control', 'required' => 'required', 'accept' => '.png , .jpg , .jpeg', 'multiple' => 'multiple']) !!}
                {!! CollectiveForm::file('image[]', null, ['class' => 'form-control', 'required' => 'required', 'accept' => '.png , .jpg , .jpeg', 'multiple' => 'multiple']) !!} --}}

                <small class="form-text text-muted">{!! trans('ticketit::lang.create-ticket-image-desc') !!}</small>
            </div>
        </div>
